#!/usr/bin/env python3
"""Main entry point for coder agent."""
...
from agents.coder import ImplementationCoder
...
async def main():
    ...
    try:
        ...
        # Create and start coder agent
        coder = ImplementationCoder(
            knowledge_base=knowledge_base,
            message_bus=message_bus,
        )
        
        logger.info("Starting Implementation Coder Agent...")
        await coder.start()
        ...
        logger.error(f"Error running coder agent: {e}")
        sys.exit(1)
